...
const LivreDorForm = ({callback}) => {
  ...
  if (!addComment) {
    ...
    return  <>
      ...
      <form className="flex w-full max-w-2xl m-auto flex-col gap-4" onSubmit={submit}>
        <div className="grid grid-flow-col justify-stretch space-x-4">
          <FloatingLabel disabled={disabled} name="email" variant="outlined" required label="Votre email" type="email" helperText="Votre email ne sera pas publié." />
          <FloatingLabel disabled={disabled} name="lname" variant="outlined" required label="Votre nom" />
          <FloatingLabel disabled={disabled} name="fname" variant="outlined" required label="Votre prénom" />
        </div>
        <div>
          <Textarea disabled={disabled} rows={5} name="comment" className="bg-white" id="comment" placeholder="Votre commentaire..." required />
        </div>
        <button disabled={disabled} className="btn btn-primary" type="submit">Envoyer mon commentaire</button>
      </form>
    </>
  }
}
...
const LivreDorComments = ({update}) => {
  ...
  if (comments.length === 0) {
    return <div className="max-w-2xl m-auto"><Spinner /></div>
  } else {
    return <Timeline className="max-w-2xl m-auto">
      {comments.sort((a, b) => {
        return (a.date < b.date) ? 1 : ((a.date > b.date) ? -1 : 0)
          ...
          <Timeline.Item key={i}>
            ...
            <Timeline.Content>
              <Timeline.Time>{
                capitalizeFirstLetter((new Date(comment.date)).toLocaleDateString("fr-FR", {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
