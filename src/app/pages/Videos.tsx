...
async function getVideos(): Promise<Video[]> {
  const results = await env.DB.prepare("SELECT * FROM videos ORDER BY sort_order ASC").all<Video>();
...
export function Videos() {
  return (
    <>
      ...
      <meta
        name="description"
        content="Découvrez les vidéos des concerts et événements de l'Harmonie Municipale de Sucy-en-Brie. Retrouvez nos performances musicales en ligne."
      />
      <meta property="og:title" content="Vidéos | Les Amis de l'Harmonie de Sucy" />
      <meta
        property="og:description"
        content="Découvrez les vidéos des concerts et événements de l'Harmonie Municipale de Sucy-en-Brie."
      />
            ...
            fallback={
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="bg-gray-100 dark:bg-gray-800 rounded-xl aspect-video animate-pulse"
                  />
                ))}
              </div>
            }
